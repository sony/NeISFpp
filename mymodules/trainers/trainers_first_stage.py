...
from mymodules.tensorutils import calc_weighted_sum, normalize_tensor, gamma
from mymodules.polarutils import calc_dolp_from_s0s1s2
...
class TrainerVolSDF(TrainerBase):
    ...
    def inference_sub(self, save_path: Path, split_size: int):
        for img_idx in range(self.data.n_images):
            ...
            result_rgb = torch.cat(result_rgb, dim=0) / self.gain
            result_srgb = gamma(result_rgb)
            ...
            images_dic = self.reshape_and_scale_all_images(
                valid_mask=mask, max_val=MAX_16BIT, srgb=result_srgb, normal=result_normal,
            )
...
class TrainerVolSDFDoP(TrainerBase):
    """ Trainer for reproducing VolSDFDoP.
    This trainer uses the DoP and RGB values to compute the loss.
    ...
    def __init__(self,
        ...
        super(TrainerVolSDFDoP, self).__init__(
            is_training=is_training,
            data_dir=data_dir,
            dataset_type=dataset_type,
            experiment_name=experiment_name,
        ...
        self.models_dic["dopnet"] = volsdf.RenderNet(
            depth=4,
            width=256,
            in_ch=6 + self.embedder_dir.out_dim + self.shape_feature_vector_size,
            out_ch=3,
        ...
        self.models_dic["rgbnet"] = volsdf.RenderNet(
            depth=4,
            width=256,
            in_ch=6 + self.embedder_dir.out_dim + self.shape_feature_vector_size,
            out_ch=3,
        ...
        if self.is_training:
            self.loss_rgb = nn.L1Loss()
            self.rgb_weight = weight_dic["rgb_weight"]
    ...
    def train_one_epoch(self, data_loader: DataLoader) -> float:
        ...
        for current_batch_num, batch in enumerate(prefetcher):
            ...
            rgb_values, z_samples_eik, _, dop_values = self.forward(rays_o, rays_d, self.is_training)

            # calc rgb loss.
            loss_s0 = self.loss_rgb(rgb_values, s0)
            loss_dop = self.loss_rgb(dop_values, dop)

            # calc Eikonal loss.
            grad_theta = self.calc_grad_theta(rays_o, rays_d, z_samples_eik)
            loss_ek = self.loss_eikonal(grad_theta)

            loss = self.rgb_weight * loss_s0 + self.eik_weight * loss_ek + self.dop_weight * loss_dop
            loss_sum += loss.item()
    ...
    def inference_sub(self, save_path: Path, split_size: int):
        for img_idx in range(self.data.n_images):
            ...
            for rays_o, rays_d in zip(rays_o_split, rays_d_split):
                ...
                rgb_values, _, normals, dop_values = self.forward(rays_o, rays_d, self.is_training)
                ...
                result_rgb.append(rgb_values.detach().cpu())
                result_dop.append(dop_values.detach().cpu())
                result_normal.append(normals.detach().cpu())

            result_rgb = torch.cat(result_rgb, dim=0) / self.gain
            result_srgb = gamma(result_rgb)
            result_dop = torch.cat(result_dop, dim=0)
            result_normal = torch.cat(result_normal, dim=0)
            ...
            images_dic = self.reshape_and_scale_all_images(
                valid_mask=mask, max_val=MAX_16BIT, srgb=result_srgb, dop=result_dop, normal=result_normal,
            )
            write_all_images_in_dict(save_path, img_idx, images_dic)
    ...
    def forward(self, rays_o: Tensor, rays_d: Tensor, is_training: bool) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        rgb_flat = self.models_dic["rgbnet"](positions, gradients, rays_d_flat, feature_vec)
        rgb = rgb_flat.reshape(-1, zn_samples, 3)  # (b_size, zn_samples, 3)

        dop_flat = self.models_dic["dopnet"](positions, gradients, rays_d_flat, feature_vec)
        dop = dop_flat.reshape(-1, zn_samples, 3)  # (b_size, zn_samples, 3)

        weights = volume_rendering(z_vals, self.models_dic["sdfnet"], sdf_val, self.device)  # (b_size, zn_samples)

        rgb_values = torch.sum(weights.unsqueeze(-1) * rgb, 1)  # (b_size, 3)
        dop_values = torch.sum(weights.unsqueeze(-1) * dop, 1)  # (b_size, 3)
        dop_values = dop_values * 0.5 + 0.5
        ...
        return rgb_values, z_samples_eik, normals, dop_values