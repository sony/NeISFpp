...
def calc_as_ap_general(cos_theta: Tensor, ref_id: Tensor) -> Tuple[Tensor, Tensor]:
    """ Compute a_s and a_p, this function supports both conductors and dielectrics.

    Refer to: https://mitsuba.readthedocs.io/en/stable/src/key_topics/polarization.html

    Args:
        cos_theta (Tensor): Cos values of the zenith angles.
        ref_id (Tensor): Refractive index, complex numbers.

    Returns:
        a_s, a_p (Tensor): reflection amplitude coefficients of perpendicular / parallel direction, complex tensors 
    """
    ref_id_rcp_sqr = (1 / ref_id) ** 2 # (n, 3)
    cos_theta_t_sqr = 1 - ref_id_rcp_sqr * (1 - cos_theta ** 2) # (n, 3)
    cos_theta_t = torch.sqrt(cos_theta_t_sqr + EPS) # (n, 3)
    cos_theta_t = cos_theta_t * torch.sign(cos_theta_t_sqr.real) # (n, 3)

    cos_theta_abs = torch.abs(cos_theta) # (n, 1)

    a_s = (cos_theta_abs - ref_id * cos_theta_t) / (cos_theta_abs + ref_id * cos_theta_t) # (n, 3)
    a_p = (ref_id * cos_theta_abs - cos_theta_t) / (ref_id * cos_theta_abs + cos_theta_t) # (n, 3)

    return a_s, a_p


def calc_rs_rp_general(a_s: Tensor, a_p: Tensor) -> Tuple[Tensor, Tensor]:
    """ Compute the perpendicular / parallel reflectance, this function supports both conductors and dielectrics.
    ...
    Returns:
        Computed R_s, R_p (Tensor).
    """
    r_s = torch.abs(a_s ** 2) # (n, 3)
    r_p = torch.abs(a_p ** 2) # (n, 3)
    return r_s, r_p


def calc_cos_delta_general(a_s: Tensor, a_p: Tensor) -> Tensor:
    """ Compute the cos value of the phase delay, this function supports both conductors and dielectrics.
    ...
    Returns:
        Computed cos(delta).
    """
    norm_s = torch.abs(a_s) # (n, 3)
    norm_p = torch.abs(a_p) # (n, 3)
    cos_delta = (a_s.real * a_p.real + a_s.imag * a_p.imag) / (norm_s * norm_p + EPS)  # (n, 3)
    return cos_delta
...
def calc_specular_stokes_mitsuba_general(wo_wld: Tensor, wi_wld: Tensor, w2c: Tensor, ref_id: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
    """ Compute specular Stokes vectors according to the view and light directions.
    ...
    Args:
        ...
        wi_wld (Tensor): light directions in world coordinate (n, 3).
        w2c (Tensor): world to camera matrix (3, 3).
        ref_id (float): refractive index, complex numbers (n, 3).

    Returns:
        specular_stokes_s0 (Tensor): the first element of the outgoing Stokes vectors (n, 3)
        specular_stokes_s1 (Tensor): the second element of the outgoing Stokes vectors (n, 3)
        specular_stokes_s2 (Tensor): the third element of the outgoing Stokes vectors (n, 3)

    Note:
        - This function assumes the incident light is not polarized.
        - This function supports both conductors (complex refractive index) and dielectrics
    """
    ...
    # calculate Rs and Rp
    cos_theta = dot_product_tensor(wo_wld, hvec_wld)  # (n, 1).
    a_s, a_p = calc_as_ap_general(cos_theta, ref_id=ref_id)  # (n, 3), (n, 3). complex numbers
    r_s, r_p = calc_rs_rp_general(a_s, a_p) # (n, 3) real numbers

    r_plus = (r_s + r_p) / 2.
    r_minus = (r_s - r_p) / 2.
    
    specular_stokes_s0 = r_plus
    specular_stokes_s1 = r_minus * cos_2phi
    specular_stokes_s2 = - r_minus * sin_2phi

    return specular_stokes_s0, specular_stokes_s1, specular_stokes_s2


def calc_specular_stokes_full_general(wo_wld: Tensor,
                                      wi_wld: torch.Tensor,
                                      cam_axis: Tensor,
                                      s0: Tensor,
                                      s1: Tensor,
                                      s2: Tensor,
                                      ref_id: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
    """ Compute Stokes vectors for specular term according to the view and light directions.
    Unlike `calc_specular_stokes_mitsuba_general()`, this function handles polarized illumination.
    ...
    Args:
        ...
        s1 (torch.Tensor): the second element of the incident Stokes vectors (n, 3).
        s2 (torch.Tensor): the third element of the incident Stokes vectors (n, 3).
        ref_id (torch.Tensor): refractive index, complex tensors.
    ...
    # calculate Rs and Rp reflectance and cos value of phase delay
    cos_theta = dot_product_tensor(wo_wld, hvec_wld)  # (n, 1).
    a_s, a_p = calc_as_ap_general(cos_theta, ref_id=ref_id)  # (n, 3). complex numbers
    r_s, r_p = calc_rs_rp_general(a_s, a_p) # (n, 3) real numbers
    cos_delta = calc_cos_delta_general(a_s, a_p) # (n, 3)

    r_plus = (r_s + r_p) / 2.
    r_minus = (r_s - r_p) / 2.
    r_cross = torch.sqrt(r_s * r_p + EPS)

    specular_stokes_s0 = (s0 * r_plus) + (s1 * r_minus) # (n, 3)
    specular_stokes_s1 = (s0 * r_minus * cos_2phi) + (s1 * r_plus * cos_2phi) + (s2 * r_cross * sin_2phi * cos_delta) # (n, 3)
    specular_stokes_s2 = (-s0 * r_minus * sin_2phi) + (-s1 * r_plus * sin_2phi) + (s2 * r_cross * cos_2phi * cos_delta) # (n, 3)

    return specular_stokes_s0, specular_stokes_s1, specular_stokes_s2

def calc_specular_fresnel_general(wo_wld: torch.Tensor,
                                  wi_wld: torch.Tensor,
                                  ref_id: Tensor) -> torch.Tensor:
    """ Compute the unpolarized fresnel calculation of the specular term.
    ...
    Returns:
        (torch.Tensor): the value of fresnel term for the specular reflection (n, 3).
